...
export default function NavComponent({ isSidebarVisible, setSidebarVisible }) {
  const sections = [
    ...
    { name: "Clases", route: "course", iconComponent: <FaCalendarAlt /> },
    { name: "Cursos", route: "course", iconComponent: <FaSchool /> },
    {
      name: "Inscripciones",
      route: "course",
      iconComponent: <FaFileSignature />,
    },
    { name: "Reportes", route: "course", iconComponent: <FaFileDownload /> },
  ];

  const adminSections = [
    { name: "Usuarios", route: "user", iconComponent: <FaUser /> },
          ...
          <div className="mt-16 text-sm text-white">
            ...
            <ul className="space-y-4 text-white">
              {sections.map((section, index) => (
                <li key={index} className=" rounded-md transition-all">
